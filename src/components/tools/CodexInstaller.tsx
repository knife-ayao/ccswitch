...
export function CodexInstaller() {
  ...
  const checkCodex = async () => {
    setIsChecking(true);
    try {
      const results = await settingsApi.getToolVersions(["codex"]);
      const codexInfo = results.find((r) => r.name === "codex");
      setToolInfo(codexInfo || null);
    } catch (error) {
      console.error("Failed to check Codex:", error);
      setToolInfo(null);
    } finally {
      setIsChecking(false);
    }
  ...
  const installCodex = async () => {
    ...
    try {
      ...
      toast.success(
        t("tools.codex.installSuccess", {
          defaultValue: "Codex 安装成功！",
        })
      );
      await checkCodex();
    } catch (error) {
      ...
      <Card>
        <CardHeader className="p-3 pb-2">
          ...
          <CardDescription className="text-xs">
            {t("tools.codex.description", {
              defaultValue: "OpenAI 官方 CLI 工具",
            })}
          </CardDescription>
        </CardHeader>
          ...
          <div className="flex items-center justify-between">
              ...
              <div>
                {isChecking ? (
                  <div className="text-xs text-muted-foreground">
                    {t("tools.codex.checking", {
                      defaultValue: "检查中...",
                    })}
                  </div>
                ) : isInstalled ? (
                  <div className="text-xs font-medium text-green-500">
                    {t("tools.codex.installed", {
            ...
            <div className="flex gap-1">
              ...
              >
                ...
                />
                {t("tools.codex.refresh", { defaultValue: "刷新" })}
              </Button>
              ...
              {!isInstalled && (
                ...
                >
                  ...
                  {isInstalling
                    ? t("tools.codex.installing", {
                        defaultValue: "安装中...",
                      })
                    : t("tools.codex.install", {
                        defaultValue: "安装",
                      })}
                </Button>
              )}
